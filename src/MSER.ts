...
export default class MSER {
  ...
  drawRectOutline(rect: Rect, rgba: RGBA, imgData: ImageData) {
    this.drawRectSolid(
      new Rect(rect.left, rect.top, rect.width, 1),
      rgba,
      imgData
    );
    this.drawRectSolid(
      new Rect(rect.left, rect.top + rect.height - 1, rect.width, 1),
      rgba,
      imgData
    );
    this.drawRectSolid(
      new Rect(rect.left, rect.top, 1, rect.height),
      rgba,
      imgData
    );
    this.drawRectSolid(
      new Rect(rect.left + rect.width - 1, rect.top, 1, rect.height),
      rgba,
      imgData
    );
  }
  ...
  extract(imageData: ImageData): Region[] {
    ...
    let processStack = function processStack(level: number, pixel) {
      let top;
